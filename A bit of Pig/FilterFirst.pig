...
-- filtering the unrequired tuples, Date is also Filtered here.
F1_filtered = FILTER F1 BY (Origin eq 'ORD' AND Destination neq 'JFK') 
		AND (Cancelled eq '0' AND Diverted eq '0')
		AND (ToDate(TravelDate,'yyyy-MM-dd') < ToDate('2008-06-01','yyyy-MM-dd')) 
		AND (ToDate(TravelDate,'yyyy-MM-dd') > ToDate('2007-05-31','yyyy-MM-dd'));
...
-- Grouping the delays and writing average on the file.

TotalResults = GROUP FilteredData all;
Average = FOREACH TotalResults GENERATE AVG(FilteredData.TotalDelay);

STORE Average into 's3://govinda.mapreduce/Pigoutput_NEW3';
